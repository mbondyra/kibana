              ...
              <EuiFlexItem grow={false}>
                <EuiButton
                  onClick={() => {
                    setLibraryVisible(false);
                    if (selectedItem) {
                      eventAnnotationService.eventAnnotationService // TODO; move this whole thing to visualizartion to avoid loading annotation service on frame level
                        .loadAnnotationGroup(selectedItem?.id)
                        .then((res) => {
                          console.log('res', res);
                        });
                    }
