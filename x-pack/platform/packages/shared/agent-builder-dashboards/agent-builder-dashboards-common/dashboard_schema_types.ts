...
export const timeRangeSchema = z.object({
  from: z.string().max(100).describe('Kibana date math (e.g. "now-24h") or ISO 8601.'),
  to: z.string().max(100).describe('"now" or ISO 8601.'),
  mode: z
    .union([z.literal('absolute'), z.literal('relative')])
    .optional()
    .describe('"relative" for date math, "absolute" for ISO 8601.'),
});
