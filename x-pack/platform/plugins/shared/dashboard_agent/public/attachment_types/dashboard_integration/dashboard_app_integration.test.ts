...
describe('registerDashboardAppIntegration', () => {
  ...
  it('reuses a locally updated existing attachment when switching to a new conversation', () => {
    const attachment = createDashboardAttachment({ origin: 'dashboard-1' });
  ...
  it('reuses the current pending attachment when switching conversations with the same draft', () => {
    register();
    emitConversationChange({ id: 'conversation-1', attachments: [] });
  ...
  it('reuses a pending attachment updated through manual changes when switching conversations', () => {
    register();
    emitConversationChange({ id: 'conversation-1', attachments: [] });
