...
export const onAttachmentMount = ({
  ...
  const apiSubscription = dashboardPlugin.dashboardAppClientApi$.subscribe((api) => {
    ...
    // layout$ emits on every panel/section layout change
    // title$ and description$ emit on metadata changes
    // TODO: when we extend query and filters and anything else, let's add it here
