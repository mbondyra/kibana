...
interface UseRegisterActionButtonsParams {
  ...
  checkForDashboardExistence: (dashboardId: string) => Promise<boolean>;
}
  ...
  useEffect(() => {
    ...
    if (dashboardApi.locator) {
      ...
      buttons.push({
        ...
        handler: async () => {
          const existingDashboardId = linkedSavedObjectIdRef.current && await checkForDashboardExistence(linkedSavedObjectIdRef.current) ? linkedSavedObjectIdRef.current : undefined;
