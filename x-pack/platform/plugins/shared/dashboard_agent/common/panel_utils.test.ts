...
import { buildMarkdownPanel, calculateMarkdownPanelHeight } from './panel_utils';
...
describe('calculateMarkdownPanelHeight', () => {
  it('should return minimum height for short content', () => {
    const height = calculateMarkdownPanelHeight('Short');
    expect(height).toBeGreaterThanOrEqual(6); // MARKDOWN_MIN_HEIGHT
  });

  it('should increase height for multi-line content', () => {
    const shortHeight = calculateMarkdownPanelHeight('Short');
    const longHeight = calculateMarkdownPanelHeight(
      'Line 1\nLine 2\nLine 3\nLine 4\nLine 5\nLine 6'
    );
  ...
  it('should cap height at maximum', () => {
    ...
    const height = calculateMarkdownPanelHeight(veryLongContent);
